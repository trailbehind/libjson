...
public:
	...
	static json_char * RemoveWhiteSpaceAndCommentsC(const json_string & value_t, bool escapeQuotes) json_nothrow json_read_priority;
	   ...
	   static JSONWGNode parse(const json_string & json) json_throws(std::invalid_argument) json_read_priority;
	   static JSONWGNode parse_unformatted(const json_string & json) json_throws(std::invalid_argument) json_read_priority;

		static JSONWGNode _parse_unformatted(const json_char * json, const json_char * const end) json_throws(std::invalid_argument) json_read_priority;

		static json_char * RemoveWhiteSpace(const json_string & value_t, size_t & len, bool escapeQuotes) json_nothrow json_read_priority;

	   static void DoArray(const internalWGJSONWGNode * parent, const json_string & value_t) json_nothrow json_read_priority;
	   static void DoNode(const internalWGJSONWGNode * parent, const json_string & value_t) json_nothrow json_read_priority;

	   #ifdef JSON_LESS_MEMORY
		  #define NAME_ENCODED this, true
		  #define STRING_ENCODED this, false
		  static json_string FixString(const json_string & value_t, const internalWGJSONWGNode * flag, bool which) json_nothrow json_read_priority;
	   #else
...
JSON_PRIVATE
	   ...
	   static void NewNode(const internalWGJSONWGNode * parent, const json_string & name, const json_string & value, bool array) json_nothrow;
    #endif
private:
    JSONWGWorker(void);
};
