...
void JSONWGNode::register_mutex_callbacks(json_mutex_callback_t lock, json_mutex_callback_t unlock, void * manager_lock) json_nothrow {
    json_lock_callback = lock;
    json_unlock_callback = unlock;
    manager_mutex = manager_lock;
}

void JSONWGNode::set_global_mutex(void * mutex) json_nothrow {
    global_mutex = mutex;
}

void JSONWGNode::set_mutex(void * mutex) json_nothrow {
    makeUniqueInternal();
    internal -> _set_mutex(mutex);
}

void * JSONWGNode::getThisLock(JSONWGNode * pthis) json_nothrow {
...
void JSONWGNode::lock(int thread) json_nothrow {
...
void JSONWGNode::unlock(int thread) json_nothrow{
...
#ifdef JSON_MUTEX_MANAGE
    void JSONWGNode::register_mutex_destructor(json_mutex_callback_t destroy) json_nothrow {
	   json_destroy = destroy;
    }
#endif


void internalWGJSONWGNode::_set_mutex(void * mutex, bool unset) json_nothrow {
...
void internalWGJSONWGNode::_unset_mutex(void) json_nothrow {
    #ifdef JSON_MUTEX_MANAGE
	   if (mylock != 0){
...
#ifdef JSON_DEBUG
    #ifndef JSON_LIBRARY
	   JSONWGNode internalWGJSONWGNode::DumpMutex(void) const json_nothrow {
		  JSONWGNode mut(JSON_NODE);
		  mut.set_name(JSON_TEXT("mylock"));
		  #ifdef JSON_MUTEX_MANAGE
			 if (mylock != 0){
				mut.push_back(JSON_NEW(JSONWGNode(JSON_TEXT("this"), (long)mylock)));
				JSON_MAP(void *, unsigned int)::iterator it = json_global(MUTEX_MANAGER).find(mylock);
				if (it == json_global(MUTEX_MANAGER).end()){
				    mut.push_back(JSON_NEW(JSONWGNode(JSON_TEXT("references"), JSON_TEXT("error"))));
				} else {
				    mut.push_back(JSON_NEW(JSONWGNode(JSON_TEXT("references"), it -> second)));
				}
			 } else {
				mut = (long)mylock;
			 }
